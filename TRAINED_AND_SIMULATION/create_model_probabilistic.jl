...
function create_model_probabilistic(probabilistic_model::Bool, alpha::Int, beta::Int, prob, samples)
     ...
     #alpha=10;
     #beta=1;
     #beta_vc = [beta, beta, beta, beta]
     #prob_vc = [prob, prob, prob, prob];
     #alpha_vc = [alpha, alpha, alpha, alpha]
        ...
        @variables(subproblem, begin
         ...
         @constraints(subproblem, begin
                ...
                [i = 1:4], demanda_es_porcen[i] == (100*demanda_es[i])/(hydro_ub[i]+thermal_ub_reservoir[i]);
                   ...
                   @constraints(subproblem, begin
                     demand_constraint[i = 1:4], sum(deficit[i, :]) + hydroGeneration[i] + sum(thermal[i, j] for j in 1:N_THERMAL[i]) +
                  sum(exchange[:, i])-sum(exchange[i, :])  >= alpha*(demand[month][i]/12000)*t+demand[month][i]+0.1*sqrt(t)*beta/100*(alpha*(demand[month][i]/12000)*t+demand[month][i])*sqrt(2)*erfinv(2*prob-1);
                   end)
                   else
                     @constraints(subproblem, begin
                     demand_constraint[i = 1:4], sum(deficit[i, :]) + hydroGeneration[i] + sum(thermal[i, j] for j in 1:N_THERMAL[i]) +
