...
function create_model_probabilistic_sample(probabilistic_model::Bool, alpha::Int, beta::Int, prob, samples)
    ...
    deficit_ub = [0.05, 0.05, 0.1, 0.8];
     ...
     demand =  Array{Float64, 2}[
        ...
        @variable(subproblem, 0<=z[i=1:4, k=1:samples]<=1, integer = true)
        ...
        @variables(subproblem, begin
            ...
            0 <= exchange[i = 1:5, j = 1:5] <= exchange_ub[i][j]  
             ...
             #0 <= deficit[i=1:4, j=1:4] <= (alpha*(demand[month][i]/12000)*t+demand[month][i])* deficit_ub[j];    
         ...
         @constraints(subproblem, begin
                ...
                [i = 1:4], demanda_es_porcen[i] == (100*demanda_es[i])/(hydro_ub[i]+thermal_ub_reservoir[i]);
                   ...
                   @constraints(subproblem, begin
                   [i=1:4], sum(deficit[i, :]) + hydroGeneration[i] + sum(thermal[i, j] for j in 1:N_THERMAL[i]) + sum(exchange[:, i])-sum(exchange[i, :])  ==  v[i];
                   [i=1:4, k=1:samples], v[i] + demandas(i,alpha,beta,samples)[t][k]*z[i,k] >= demandas(i,alpha,beta,samples)[t][k];
                   [i=1:4], sum(z[i, k] for k in 1:samples) <= N;
                   end)
                     ...
                     @constraints(subproblem, begin
                     demand_constraint[i = 1:4], sum(deficit[i, :]) + hydroGeneration[i] + sum(thermal[i, j] for j in 1:N_THERMAL[i]) +
                     sum(exchange[:, i])-sum(exchange[i, :]) == demanda_es[i];
                     #[i = 1:4, j = 1:4], deficit[i,j] <= demanda_es[i]*deficit_ub[j];
                      end)
                   end
